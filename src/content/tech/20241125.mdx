...
So actually, no additional work is required in the Nuxt3 source.<br />
...
As mentioned earlier, no additional settings are required `in the source`.<br />
Add the following build command or Environment variable when deploying to Vercel.
...
I usually set the build command as follows.<br />
(Since I'm using Bun, I added `--preset=vercel-edge` to the command!)
